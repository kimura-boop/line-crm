...
function detectLiffId(): string {
  ...
  if (import.meta.env?.VITE_LIFF_ID) return import.meta.env.VITE_LIFF_ID;
  // Injected by Worker at serve time via window.__LIFF_ID__
  return (window as unknown as { __LIFF_ID__?: string }).__LIFF_ID__ || '';
}
...
function getPage(): string | null {
  ...
  if (path === 'book' || path === 'liff/book') return 'book';
  if (path === 'liff/register') return 'register';
  if (path === 'liff/form') return 'form';
...
async function main() {
  if (!LIFF_ID) {
    showError('LIFF ID が設定されていません。管理者にお問い合わせください。');
    return;
  }

  // LIFF SDK ロード失敗チェック
  if ((globalThis as Record<string, unknown>).__LIFF_SDK_ERROR__) {
    showError('LINE SDKの読み込みに失敗しました。LINEアプリを再起動してください。');
    return;
  }
  if (typeof (globalThis as Record<string, unknown>).liff === 'undefined') {
    showError('LINE SDKが読み込まれていません。LINEアプリを再起動してください。');
    return;
  }

  try {
    // liff.init() に15秒のタイムアウトを設定
    await Promise.race([
      liff.init({ liffId: LIFF_ID }),
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error('LIFF初期化がタイムアウトしました。LINEアプリを再起動してください。')),
          15000,
        )
      ),
    ]);
