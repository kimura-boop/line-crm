...
webhook.post('/webhook', async (c) => {
  ...
  // 環境変数から直接認証情報を取得（DBルックアップをスキップ）
  const channelSecret = c.env.LINE_CHANNEL_SECRET ?? '';
  const channelAccessToken = c.env.LINE_CHANNEL_ACCESS_TOKEN ?? '';
  const matchedAccountId: string | null = null;

  console.log('[webhook] channelSecret length:', channelSecret.length);
  ...
  // Verify with env secret directly
